...
_config = load_config()
DEFAULT_MODEL_NAME = get_config_value(
    _config, "training", "transformer", "model_name", default="distilbert-base-uncased"
)
DEFAULT_MAX_LENGTH = get_config_value(
    _config, "training", "transformer", "max_length", default=128
)
DEFAULT_BATCH_SIZE = get_config_value(
    _config, "training", "transformer", "batch_size", default=16
)
DEFAULT_NUM_EPOCHS = get_config_value(
    _config, "training", "transformer", "num_epochs", default=3
)
DEFAULT_LEARNING_RATE = get_config_value(
    _config, "training", "transformer", "learning_rate", default=2e-5
)
...
class TransformerSentimentTrainer:
    ...
    def prepare_dataset(
        ...
        self.dataset = self.dataset.map(
            tokenize_fn, batched=True, remove_columns=[text_column]
        )
