...
_config = load_config()
DEFAULT_MAX_FEATURES = get_config_value(_config, 'model', 'classical', 'max_features', default=5000)
...
class ClassicalSentimentModel:
    ...
    def fit(self, X_train, y_train, feature_params: Dict[str, Any] = None):
        ...
        if isinstance(X_train, (list, pd.Series)) and isinstance(X_train.iloc[0] if isinstance(X_train, pd.Series) else X_train[0], str):
    ...
    def predict(self, X):
        ...
        if self.feature_extractor and isinstance(X, (list, pd.Series)) and isinstance(X.iloc[0] if isinstance(X, pd.Series) else X[0], str):
    ...
    def save(self, model_path: str, feature_extractor_path: str = None, model_params: Dict = None):
...
def train_classical_model(
    ...
    df = load_and_prepare_data(data_path, sample_frac=DEFAULT_SAMPLE_FRACTION, random_state=random_state)
    ...
    logger.info(f"Training Logistic Regression with TF-IDF features (max_features={max_features})...")
