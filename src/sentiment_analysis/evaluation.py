...
def evaluate_classification(
    ...
    logger.info(
        f"Evaluation complete: accuracy={accuracy:.4f}, f1_macro={f1_macro:.4f}"
    )
...
def compare_models(
    ...
    for model_name, y_pred in models_predictions.items():
        ...
        metrics = evaluate_classification(
            y_true, y_pred, label_names=label_names, verbose=False
        )
    ...
    logger.info("=" * 80)
    logger.info(
        f"{'Model':<30} {'Accuracy':>10} {'F1-Macro':>10} {'Precision':>10} {'Recall':>10}"
    )
    logger.info("-" * 80)

    for model_name, metrics in results.items():
        logger.info(
            f"{model_name:<30} {metrics['accuracy']:>10.4f} {metrics['f1_macro']:>10.4f} "
