...
class SentimentPredictor:
    ...
    def _load_transformer_model(self):
        ...
        logger.info(f"Loaded transformer model from {self.model_path} ({num_labels} labels)")
    ...
    def _predict_transformer(
        ...
        if isinstance(text, str):
            if return_probabilities:
                mapped_preds = [{'label': self.label_map[p['label']], 'score': p['score']} for p in results]
            ...
            if return_probabilities:
                return [
                    {'text': t, 'predictions': [{'label': self.label_map[p['label']], 'score': p['score']} for p in r]}
                    for t, r in zip(text, results)
                ]
            else:
                return [
                    {'text': t, 'label': self.label_map[r[0]['label']], 'score': r[0]['score']}
                    for t, r in zip(text, results)
                ]
    ...
    def _predict_classical(
        ...
        # Predict
        predictions = self.model.predict([cleaned_texts[0]] if isinstance(text, str) else cleaned_texts)
        
        # Get probabilities (always get them if model supports it)
        if hasattr(self.model, 'predict_proba'):
            probas = self.model.predict_proba([cleaned_texts[0]] if isinstance(text, str) else cleaned_texts)
        else:
            probas = None
        ...
        if isinstance(text, str):
            ...
            for i, t in enumerate(texts):
                results.append({
                    ...
                    'score': probas[i][predictions[i]] if probas is not None else None
                })
            return results
