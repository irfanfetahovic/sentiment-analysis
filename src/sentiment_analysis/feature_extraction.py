...
from sentiment_analysis.exceptions import ModelNotFittedError, ModelLoadError, ModelSaveError
...
_ngram_range_list = get_config_value(_config, 'model', 'classical', 'ngram_range', default=[1, 1])
...
class TFIDFFeatureExtractor:
    ...
    def __init__(self, max_features: int = DEFAULT_MAX_FEATURES, ngram_range: Tuple[int, int] = DEFAULT_NGRAM_RANGE):
        """
        ...
        logger.debug(f"Initialized TFIDFFeatureExtractor (max_features={max_features}, ngram_range={ngram_range})")
    ...
    def save(self, path: str):
        ...
        try:
            path = Path(path)
            path.parent.mkdir(parents=True, exist_ok=True) # create directories if not exist and don't raise error if they do
