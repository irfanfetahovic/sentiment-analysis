...
def compare_multiple_models(
    ...
    if num_labels is None:
        ...
        num_labels = len(np.unique(y_test))
        print(
            f"Warning: num_labels not found in model configs, detected from test data"
        )
    ...
    results = compare_models(
        models_predictions=all_predictions, y_true=y_test, label_names=label_names
    )
    ...
    if output_file:
        ...
        for model_name, metrics in results.items():
            ...
            if "confusion_matrix" in metrics:
                results_to_save[model_name]["confusion_matrix"] = metrics[
                    "confusion_matrix"
                ]
...
Examples:
  ...
  python scripts/run_evaluate.py --model-path models/distilbert --data-path data/Reviews.csv --test-size 0.3
    ...
    # Model arguments
    parser.add_argument(
        "--model-path", type=str, help="Path to model for single evaluation"
    )
    parser.add_argument(
        "--model-type",
        type=str,
    ...
    parser.add_argument(
        "--test-size", type=float, help="Fraction of data to use for testing"
    )
    parser.add_argument(
        "--sample-frac",
        type=float,
    ...
    # Output arguments
    parser.add_argument(
        "--output-file", type=str, help="Path to save evaluation results as JSON"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    try:
        df = load_and_prepare_data(
            data_path, sample_frac=sample_frac, skip_preprocessing=True
        )
