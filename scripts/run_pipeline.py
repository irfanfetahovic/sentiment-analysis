...
class PipelineRunner:
    ...
    def train_classical_model(
        ...
        returncode, stdout, stderr, duration = self.run_script('run_train_classical.py', args)
    ...
    def train_transformer_model(
        ...
        args = [
            '--data-path', self.data_path,
            '--output-dir', str(Path(self.models_dir) / 'distilbert_sentiment'),
        ...
        returncode, stdout, stderr, duration = self.run_script('run_train_transformer.py', args)
    ...
    def compare_models(self, models_config_path: str) -> bool:
        ...
        args = [
            '--compare',
            '--models-json', models_config_path,
            '--data-path', self.data_path,
            '--output-file', output_file
        ]
...
Examples:
  ...
  python run_pipeline.py --train-all --sample-frac 0.1 --num-epochs 2
    ...
    parser.add_argument('--train-classical', action='store_true',
                        help='Train only classical model')
    ...
    parser.add_argument('--compare', action='store_true',
                        help='Compare models after training')
    parser.add_argument('--models-json', type=str,
                        help='Path to models config for comparison')
    ...
    if not args.skip_training and not (train_classical or train_transformer):
        print("Error: Must specify --train-all, --train-classical, --train-transformer, or --skip-training")
        sys.exit(1)
    ...
    if args.compare:
        ...
        if not Path(models_json).exists():
            ...
            classical_path = str(Path(args.models_dir) / 'classical_models' / 'logistic_tfidf_model.pkl')
